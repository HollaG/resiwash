...
import { sendOkResponse } from "../../core/responses";
...
// for hardware only
export const registerSensor = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (existingSensor) {
    res.status(400).json({ error: "Sensor with this macAddress already exists" });
    return;
  }
...
export const setSensorApiKey = asyncHandler(async (req: Request, res: Response) => {
  ...
  const sensorRepository = AppDataSource.getRepository(Sensor);
  const sensor = await sensorRepository.findOneBy({ sensorId: Number(req.params.id) });

  const room = await AppDataSource.getRepository("Room").findOneBy({ roomId: Number(roomId) });
  ...
  if (apiKey != null)
    sensor.apiKey = apiKey;

  if (roomId != null)
    sensor.room = { roomId: Number(roomId) } as any; // Type assertion to match the Room type
  await sensorRepository.save(sensor);
...
export const getSensorLinks = asyncHandler(async (req: Request, res: Response) => {
  ...
  const sensorToMachineRepository = AppDataSource.getRepository(SensorToMachine);
...
export const setSensorLink = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (source === undefined || localId === undefined || machineId === undefined) {
    res.status(400).json({ error: "source, localId, and machineId are required" });
    return;
  }


  // validate sensor existence
  const sensor = await AppDataSource.getRepository(Sensor).findOneBy({ sensorId: Number(req.params.id) });
  if (!sensor) {
    res.status(404).json({ error: "Sensor not found" });
    return;
  }
  ...
  const machine = await AppDataSource.getRepository("Machine").findOneBy({ machineId: Number(machineId) });
  if (!machine) {
...
export const deleteSensorLink = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!source || localId === undefined || machineId === undefined) {
    res.status(400).json({ error: "source, localId or machineId are required" });
    return;
  }
