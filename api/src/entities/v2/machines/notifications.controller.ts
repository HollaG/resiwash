...
import { claimMachine as _claimMachine, unclaimMachine as _unclaimMachine, getClaimants } from "../../../utils/notifications";
...
export const claimMachine = expressAsyncHandler(async (req: Request<{ machineId: string }, unknown, unknown, ClaimMachineRequest>, res: Response) => {
  try {
    const { machineId } = req.params;
    const { fcmToken, cycleTime } = req.body as ClaimMachineRequest;

    // first, check for valid machineId in DB
    const machine = await AppDataSource.getRepository("Machine").findOneBy({ machineId: parseInt(machineId) });
...
export const unclaimMachine = expressAsyncHandler(async (req: Request, res: Response) => {
  try {
    const { machineId } = req.params;
...
export const pokeClaimant = expressAsyncHandler(async (req: Request, res: Response) => {
  try {
    const { machineId } = req.params;
