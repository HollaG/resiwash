...
class TrackerMiniAlt extends StatefulWidget {
  final MachineEntity machine;
  final ClaimedMachineMetadata claimedMetadata;
  const TrackerMiniAlt({
    super.key,
  ...
  @override
  State<TrackerMiniAlt> createState() => _TrackerMiniAltState();
}

class _TrackerMiniAltState extends State<TrackerMiniAlt>
  ...
  int calculatePercentDone() {
    ...
    if (lastAvailableTime != null) {
      ...
      final percentDone =
          (timeSinceAvailable.inSeconds / totalCycleDuration.inSeconds).clamp(
            0,
            1,
          );

      return (percentDone * 100).toInt();
    }
          ...
          child: Row(
            ...
            children: [
              CircularProgressIndicator(
                value: calculatePercentDone() / 100,
                year2023: false,
                color: context.appColors.inUse.colorContainer,
                backgroundColor: context.appColors.inUse.onColorContainer,
              ...
              // MachineStatusIndicator(status: widget.machine.currentStatus),
              Text(
                "${widget.machine.name} @ ${widget.machine.room?.area?.shortName ?? widget.machine.room?.area?.name}",
